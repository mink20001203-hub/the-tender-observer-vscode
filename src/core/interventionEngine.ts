import { ObserverSettings, RhythmState, TriggerVariant, WhisperDecision } from "./types";

function canWhisper(lastWhisperAt: number, now: number, cooldownMinutes: number): boolean {
  return now - lastWhisperAt >= cooldownMinutes * 60_000;
}

export function whisperForState(
  params: {
    state: RhythmState;
