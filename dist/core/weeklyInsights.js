...
exports.buildWeeklyInsights = buildWeeklyInsights;
...
function countByHourRange(snapshots, state, startHourInclusive, endHourExclusive) {
    return snapshots.filter((snapshot) => snapshot.state === state && snapshot.hour >= startHourInclusive && snapshot.hour < endHourExclusive).length;
}
function buildWeeklyInsights(payload) {
    const insights = [];
